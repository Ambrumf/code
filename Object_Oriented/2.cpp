...
{
    while(src.find(delchar)!=-1)
    {
        src.erase(src.find(delchar),1);
    }
    return src;
}
string remove(string src,int index)
{
    if(index <0 || index > src.size()-1)return src;
    src.erase(index,1);
    return src;
}
string remove(string src,int startIndex,int endIndex)
{
    if(startIndex <0 || startIndex > src.size()-1) return src;
    if(endIndex <0 || endIndex > src.size()-1) return src;
    int n=endIndex-startIndex+1;
    src.erase(startIndex,n);
    return src;
}
int main()
{
