...
int f[9],vis[362882]={},step[362882]={};
struct node{
    int num,step;
...
int main()
{
    ...
    q1.push(st);q2.push(ed);vis[contor(tt)]=1;vis[contor(des)]=2;step[contor(tt)]=0;step[contor(des)]=0;
        ...
        while(!q1.empty()){
            ...
            for(int i=0;i<4;i++){
                ...
                if(inboard(nx,ny)){
                    ...
                    int nnum=toN();
                    temp=contor(nnum);
                    if(!vis[temp]){vis[temp]=1;step[temp]=st.step+1;tmpq.push(node(nnum,st.step+1));
                    }
                    else if(vis[temp]==2){ans=step[temp]+1+st.step;cout<<ans;exit(0);}
                    swap(matrix[x][y],matrix[nx][ny]);
                }
            }
        }
        ...
        while(!q2.empty()){
            ...
            for(int i=0;i<4;i++){
                ...
                if(inboard(nx,ny)){
                    ...
                    if(!vis[temp]){vis[temp]=2;step[temp]=st.step+1;tmpq.push(node(nnum,st.step+1));
                    //cout<<nnum<<endl;
                    }
                    else if(vis[temp]==1){ans=step[temp]+1+st.step;cout<<ans;exit(0);}
                    swap(matrix[x][y],matrix[nx][ny]);
                }
            }
        }
